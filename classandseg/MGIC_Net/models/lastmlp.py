...
class MV3Dunet_down_lastmlp(nn.Module):
    def __init__(self, num_classes, attention=True, in_channels=3, feature_scale=4, is_batchnorm=False,
        ...
        self.classifier = nn.Sequential(nn.Linear(30 + 30 + 188, 128),nn.ReLU(),nn.Linear(128,32),nn.ReLU(),nn.Linear(32,num_classes))
    ...
    def forward(self, x, oneHot, num):  # x:(batch) * 3(views) * 3 rgb channel*27(depth) *im_size* im_size
        ...
        for v in x:  # 3 views
            ...
            t_num = self.catnum_tensor(num, self.num_conv1d, self.num_batch_norm, 57, 30, [10, 3, 3]) #torch.Size([2, 60, 10, 10, 10])
