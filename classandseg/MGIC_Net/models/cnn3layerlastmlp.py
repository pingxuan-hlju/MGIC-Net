...
class cnn3layerlastmlp(nn.Module):
    def __init__(self, num_classes, attention=True, in_channels=3, feature_scale=4, is_batchnorm=True,
        ...
        filters = [int(x / self.feature_scale) for x in filters]  # 每个block输出的通道数量
        ...
        self.classifier = nn.Sequential(nn.Linear(30 + 30 + 124, 128),nn.ReLU(),nn.Linear(128,32),nn.ReLU(),nn.Linear(32,num_classes))
    ...
    def forward(self, x, oneHot, num):  # x:(batch) * 3(views) * 3 rgb channel*27(depth) *im_size* im_size
        ...
        for v in x:  # 3 views
            ...
            t_onehot = self.catnum_tensor(oneHot, self.oh_conv1d, self.oh_batch_norm, 83, 30,
                                          [40, 12, 12])  # shape:torch.Size([2, 30, 10, 10, 10])
            t_num = self.catnum_tensor(num, self.num_conv1d, self.num_batch_norm, 57, 30, [40, 12, 12]) #torch.Size([2, 60, 10, 10, 10])

            in_feat5 = torch.cat([conv3, t_onehot, t_num], 1)  #torch.Size([2, 218, 10, 10, 10])
            in_feat5 = in_feat5.type(torch.cuda.FloatTensor)  # .to('cuda')
            cmsa_gconv3 = self.CMSA_layer(in_feat5, dim=124, conv=self.conv_cmsa3, maxpool=self.maxpool_cmsa,
            ...
            # mvcnn style
            pooled = torch.sum(conv3.view(batch_size, conv3.shape[1], -1), dim=-1)
