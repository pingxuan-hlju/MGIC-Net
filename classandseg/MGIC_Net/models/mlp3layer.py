...
class mlp3layer(nn.Module):
    def __init__(self, num_classes, attention=True, in_channels=3, feature_scale=4, is_batchnorm=True,
        ...
        self.classifier = nn.Sequential(nn.Linear(32 + 32 + 128, 2))
    ...
    def forward(self, x, oneHot, num):  # x:(batch) * 3(views) * 3 rgb channel*27(depth) *im_size* im_size
        ...
        for v in x:  # 3 views
            ...
            t_onehot = self.catnum_tensor(oneHot, self.oh_linear, self.oh_batch_norm, 83, 32,[40, 12, 12])  # shape:torch.Size([2, 30, 10, 10, 10])
            t_num = self.catnum_tensor(num, self.num_linear, self.num_batch_norm, 57, 32, [40, 12, 12]) #torch.Size([2, 60, 10, 10, 10])
