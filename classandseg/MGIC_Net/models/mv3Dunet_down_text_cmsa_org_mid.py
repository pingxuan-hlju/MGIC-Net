...
import numpy as np
class mv3Dunet_down_text_cmsa_org_mid(nn.Module):
    def __init__(self, num_classes, attention=True, in_channels=1, feature_scale=4,is_batchnorm=True,
        ...
        super(mv3Dunet_down_text_cmsa_org_mid, self).__init__()
        ...
        self.oh_conv1d = torch.nn.Conv1d(24, 1568, kernel_size=1, stride=1)
        # self.maxpool1d = torch.nn.MaxPool1d(kernel_size=1)
        self.avg_pool1d = torch.nn.AvgPool1d(kernel_size=1)
        self.oh_batch_norm = torch.nn.BatchNorm1d(1568)

        self.num_conv1d = torch.nn.Conv1d(11, 1568, kernel_size=1, stride=1)
        self.num_batch_norm = torch.nn.BatchNorm1d(1568)
        ...
        self.gcn_mlp1 = nn.Linear(1568,1568)
        self.gcn_mlp2 = nn.Linear(1568,1568)
        self.emb_1 = nn.Linear(1568,1568)
        self.emb_2 = nn.Linear(1568,1568)
        self.final_1 = nn.Linear(1568,1568)
        self.final_2 = nn.Linear(1568,1568)
        self.final_3 = nn.Linear(1568,1568)
        self.final_4 = nn.Linear(1568,1568)
        self.norm1 = nn.LayerNorm(1568, eps=1e-6)
        self.norm2 = nn.LayerNorm(1568, eps=1e-6)
    ...
    def forward(self, x,oneHot,num):
        ...
        t_onehot = self.catnum_tensor(oneHot, self.oh_conv1d, self.oh_batch_norm, 24,30, [8, 14, 14])
        t_num = self.catnum_tensor(num, self.num_conv1d, self.num_batch_norm, 11, 15,[8, 14, 14])
