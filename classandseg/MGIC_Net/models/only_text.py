...
class OnlyText(nn.Module):
    def __init__(self, num_classes, attention=True, in_channels=3, feature_scale=4, is_batchnorm=False,
        ...
        self.oh_conv1d = torch.nn.Conv1d(84, 30, kernel_size=1, stride=1)
        ...
        self.classifier = nn.Linear(30 + 30+60, num_classes)
    ...
    def forward(self, x, oneHot, num):  # x:(batch) * 3(views) * 3 rgb channel*27(depth) *im_size* im_size
        ...
        for v in x:  # 3 views
            # Feature Extraction
            t_onehot = self.catnum_tensor(oneHot, self.oh_conv1d, self.oh_batch_norm, 84, 30,
                                          [10, 10, 10])  # shape:torch.Size([2, 30, 10, 10, 10])
            t_num = self.catnum_tensor(num, self.num_conv1d, self.num_batch_norm, 57, 30, [10, 10, 10]) #torch.Size([2, 60, 10, 10, 10])

            in_feat5 = torch.cat([t_onehot, t_num], 1)  #torch.Size([2, 218, 10, 10, 10])
            in_feat5 = in_feat5.type(torch.cuda.FloatTensor)  # .to('cuda')
            cmsa_gconv3 = self.CMSA_layer(in_feat5, dim=60, conv=self.conv_cmsa3, maxpool=self.maxpool_cmsa,
        ...
        g1_view = torch.max(g1_view, view_g1[0])
        g2_view = torch.max(g2_view, view_g2[0])
        pooled_cmsa_view = torch.max(pooled_cmsa_view, view_cmsa[0])
