...
class ImageAttention(nn.Module):
    ...
    def forward(self, x, oneHot, num):  # x:(batch) * 3(views) * 3 rgb channel*27(depth) *im_size* im_size
        ...
        for v in x:  # 3 views
            ...
            in_feat5 = torch.cat([conv5], 1)  #torch.Size([2, 218, 10, 10, 10])
            in_feat5 = in_feat5.type(torch.cuda.FloatTensor)  # .to('cuda')
            cmsa_gconv3 = self.CMSA_layer(in_feat5, dim=128, conv=self.conv_cmsa3, maxpool=self.maxpool_cmsa,
        ...
        pooled_view = self.aggregation_concat(pooled_cmsa_view) 
