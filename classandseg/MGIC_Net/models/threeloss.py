...
class Threeloss(nn.Module):
    def __init__(self, num_classes, attention=True, in_channels=3, feature_scale=4,is_batchnorm=True,
        ...
        self.oh_conv1d1 = torch.nn.Conv1d(57, 30, kernel_size=1, stride=1)
        self.oh_conv1d2 = torch.nn.Linear(30, num_classes)
        ...
        self.num_conv1d1 = torch.nn.Conv1d(57, 30, kernel_size=1, stride=1)
        self.num_conv1d2 = torch.nn.Linear(30, 2)
        ...
        self.classifier=nn.Linear(30+30+188, num_classes)
    ...
    def catnum_tensor(self, t, linear_encoder1,linear_encoder2, batch_norm, attr,out_attr, in_shape):

        # self.nn_out = torch.nn.Linear(attr, int(attr/2))
        # t = linear_encoder(t.float())

        t = torch.reshape(t, [t.shape[0], attr, 1])  # [b,72,1]

        t = linear_encoder1(t.float())
        ...
        silu = nn.SiLU()
        t = silu(t)
        res = linear_encoder2(t.reshape(t.shape[0],-1))
        #t = self.avg_pool1d(t)

        t = t.repeat(1, 1, np.prod(in_shape))  # [b,72,9]

        t = torch.reshape(t, [t.shape[0], out_attr, in_shape[0], in_shape[1], in_shape[2]])  # [b,72, 1, 3, 3]
    ...
    def forward(self, x,oneHot,num):
        ...
        for v in x:  # 3 views
            ...
            t_onehot,res_onehot = self.catnum_tensor(oneHot, self.oh_conv1d1,self.oh_conv1d2 ,self.oh_batch_norm, 57,30, [10, 3, 3])
            t_num,res_num = self.catnum_tensor(num, self.num_conv1d1, self.num_conv1d2,self.num_batch_norm, 57, 30,[10, 3, 3])

            in_feat5 = torch.cat([conv5, t_onehot, t_num], 1)  # [b,207,1,3,3] # add t_onehot,
            in_feat5 = in_feat5.type(torch.cuda.FloatTensor)  # .to('cuda')
            cmsa_gconv3 = self.CMSA_layer(in_feat5, dim=188, conv=self.conv_cmsa3, maxpool=self.maxpool_cmsa,
        ...
        return pooled_view,res_onehot,res_num
