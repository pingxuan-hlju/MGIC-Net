...
import copy
import yaml
class MV3Dunet_down_text_cmsa_small(nn.Module):
    def __init__(self, num_classes, attention=True, in_channels=3, feature_scale=4, is_batchnorm=True,
        ...
        super(MV3Dunet_down_text_cmsa_small, self).__init__()
        ...
        self.oh_conv1d = torch.nn.Conv1d(71, 30, kernel_size=1, stride=1)
        ...
        self.num_conv1d = torch.nn.Conv1d(7, 30, kernel_size=1, stride=1)
    ...
    #     abs_diff = torch.abs(tensor.unsqueeze(2) - tensor.unsqueeze(1))

    #     # # 计算绝对值的差之和（L1 距离）
    #     distances = torch.sum(abs_diff, dim=3)

    #     # # 计算指数
    #     L1 = torch.exp(-distances)
    #     # # 计算度矩阵 D
    #     # A = nn.functional.softmax(L1,dim=-1)
    #     D = torch.diag_embed(torch.sum(L1, dim=2))+ 0.01
    #     # 计算 D^(-1/2)
    #     D_sqrt_inv = torch.sqrt(torch.inverse(D))
    #     A = torch.matmul(torch.matmul(D_sqrt_inv, L1), D_sqrt_inv) 
    #     # A = nn.functional.softmax(torch.matmul(tensor, tensor.transpose(-1,-2)),dim=-1)
    #     output = relu(mlp(torch.matmul(L1, tensor)))
