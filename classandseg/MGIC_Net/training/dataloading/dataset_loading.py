...
class RandomUniqueBatchIterator:
    def __init__(self, iterable, batch_size):
        self.iterable = iterable
        self.batch_size = batch_size
        self.remaining_items = list(iterable)
        self.current_batch = []

    def _get_next_batch(self):
        random.shuffle(self.remaining_items)
        self.current_batch = self.remaining_items[:self.batch_size]
        self.remaining_items = self.remaining_items[self.batch_size:]
    ...
    def __next__(self):
        if not self.remaining_items:
            self.remaining_items = list(self.iterable)
...
class DataLoader3D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
        ...
        self.iterator = RandomUniqueBatchIterator(self.list_of_keys, self.batch_size)
    ...
    def generate_train_batch(self):
        ...
        # selected_keys = random.sample(self.list_of_keys, self.batch_size)

        selected_keys = [next(self.iterator) for _ in range(self.batch_size)]
