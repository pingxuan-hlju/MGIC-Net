...
class NetworkTrainer(object):
    def __init__(self, deterministic=True, fp16=False):
        ...
        self.num_batches_per_epoch = 366
        self.num_val_batches_per_epoch = 92
        ...
        self.best_f1_class = 0
        self.best_idx = 0
        self.val_f1_class_MA = 0
        self.val_sens_class_MA = 0
    ...
    def save_checkpoint(self, fname, save_optimizer=True):
        ...
        state_dict4 = self.network4.state_dict()
        ...
        for key in state_dict4.keys():
            state_dict4[key] = state_dict4[key].cpu()
    ...
    def load_checkpoint_ram(self, checkpoint, train=True):
        ...
        new_state_dict4 = OrderedDict()
        curr_state_dict_keys = list(self.network.state_dict().keys())
        curr_state_dict_keys1 = list(self.network1.state_dict().keys())        
        curr_state_dict_keys2 = list(self.network2.state_dict().keys())
        curr_state_dict_keys4 = list(self.network4.state_dict().keys())
        ...
        for k, value in checkpoint['state_dict2'].items():
            key = k
            if key not in curr_state_dict_keys2 and key.startswith('module.'):
                key = key[7:]
            new_state_dict2[key] = value                              
        for k, value in checkpoint['state_dict4'].items():
            key = k
            if key not in curr_state_dict_keys4 and key.startswith('module.'):
                key = key[7:]
        ...
        self.network4.load_state_dict(new_state_dict4)
    ...
    def run_training(self):
        ...
        while self.epoch < self.max_num_epochs:
            ...
            print("\nepoch %d learning rate %f\n" %
                  (self.epoch, self.optimizer1.param_groups[0]['lr']))
            ...
            with torch.no_grad():
                ...
                score_test_pre = []  # class
                for b in range(self.num_val_batches_per_epoch):
                    l,cls_evl = self.run_iteration(self.val_gen, False, True)                    
                    """
                    class
                    """
                    total += self.batch_size
                    correct += cls_evl[0]
                    score_test_pre.extend(cls_evl[1])
                    test_predictions.extend(cls_evl[2])
                    test_labels.extend(cls_evl[3])
                    """
                    class
                    """
                    ...
                    val_losses.append(l)
                """class"""
                print("\n[epoch %d] accuracy on test data: %.2f%%\n" %(self.epoch, 100 * correct / total))
                test_acc = metrics.accuracy_score(test_labels, test_predictions)
                score_test_pre = np.array(score_test_pre)
                try:
                    test_auc = metrics.roc_auc_score(test_labels, score_test_pre[:, 1], average='weighted') # 报错
                except:
                    test_auc = 0
                try:
                    test_ave_precision = metrics.average_precision_score(test_labels, score_test_pre[:, 1], average='weighted') # 报错
                except:
                    test_ave_precision = 0
                ...
                F1 = metrics.f1_score(test_labels, test_predictions)
                self.val_f1_class_MA = F1
                self.val_sens_class_MA = test_sens
                self.print_to_log_file("test_acc:", test_acc, '\n', "test_sens:", test_sens, '\n', "test_auc:", test_auc, '\n',
                  "test_ave_precision:", test_ave_precision, '\n', "test_F1:", F1, '\n',
                  "test_recall:", recall, '\n', "test_precision:", precision, '\n', "test_spec :", test_spec)
                if self.wandb:
                    wandb.log({"test/test_acc": test_acc, "test/test_auc": test_auc,
                       "test/accuracy": correct / total, "test/test_average_precision": test_ave_precision,
                       'test/recall': recall, 'test/precision': precision, 'test/test_sens': test_sens, 'test/test_spec': test_spec, 'test/F1': F1})
    ...
    def manage_patience(self):
        ...
        if self.patience is not None:
            ...
            if self.val_f1_class_MA > self.best_f1_class:
                self.best_f1_class = self.val_f1_class_MA
                self.print_to_log_file("saving best class epoch checkpoint...")
                if self.save_best_checkpoint: self.save_checkpoint(join(self.output_folder, f"model_best_class_{self.best_idx}.model"))
                self.best_idx += 1

            if self.val_f1_class_MA > 0.6:
                self.print_to_log_file("saving class epoch checkpoint...")
                if self.save_best_checkpoint: self.save_checkpoint(join(self.output_folder, f"model_f1_class_{self.val_f1_class_MA}_{self.epoch}.model"))    
            if self.val_sens_class_MA > 0.6 and self.epoch > 50:
                self.print_to_log_file("saving class epoch checkpoint...")
                if self.save_best_checkpoint: self.save_checkpoint(join(self.output_folder, f"model_sens_class_{self.val_sens_class_MA}_{self.epoch}.model"))         
