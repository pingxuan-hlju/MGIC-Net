...
class nnUNetTrainer(NetworkTrainer):
    ...
    def plot_network_architecture(self):
        try:
            ...
            if self.wandb:
                ...
                wandb.init(project="segandclass_Esophagus")
                wandb.watch(self.network1, log="all")  
        finally:
            if torch.cuda.is_available():
    ...
    def process_plans(self, plans):
        ...
        # self.patch_size = np.array([160,160,160])
    ...
    def predict_preprocessed_data_return_seg_and_softmax(self, data: Tuple, do_mirroring: bool = True,
        ...
        data,one_hot,num=data
        current_mode = self.network.training
        self.network.eval()
        self.network1.eval()
        self.network2.eval()
        self.network4.eval()
        ret,small,mid,count_tensor = self.network.predict_3D(data, do_mirroring=do_mirroring, mirror_axes=mirror_axes,
                                      use_sliding_window=use_sliding_window, step_size=step_size,
                                      patch_size=self.patch_size, regions_class_order=self.regions_class_order,
                                      use_gaussian=use_gaussian, pad_border_mode=pad_border_mode,
                                      pad_kwargs=pad_kwargs, all_in_gpu=all_in_gpu, verbose=verbose,
                                      mixed_precision=mixed_precision,seg=True)
        mid = mid.to(torch.float32)
        small = small.to(torch.float32)
        with torch.no_grad():
        ...
        self.network1.train(current_mode)
        self.network.train(current_mode)
        self.network2.train(current_mode)
        self.network4.train(current_mode)
        return ret,pre
    ...
    def run_online_evaluation(self, output, target):
        with torch.no_grad():
            num_classes = output.shape[1]  # 有多少个类：2
            output_softmax = softmax_helper(output)  # softmax
            output_seg = output_softmax.argmax(1)  # 取分割区域
            target = target[:, 0]
