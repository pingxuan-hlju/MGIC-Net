...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
    ...
    for preprocessed in preprocessing:
        ...
        num=[]
        label=[]
        onehotpath = os.path.dirname(list_of_lists[0][0])+'/one_hot.csv'
        numpath = os.path.dirname(list_of_lists[0][0])+'/num.csv'
        idx = int(re.search(r'(?<=_)\d+(?=_)', os.path.basename(dct['list_of_data_files'][0])).group(0))
        with open(onehotpath, newline='') as csvfile:
        ...
        softmax,pred_test = trainer.predict_preprocessed_data_return_seg_and_softmax(
            d,do_mirroring=do_tta, mirror_axes=trainer.data_aug_params['mirror_axes'], use_sliding_window=True,
            step_size=step_size, use_gaussian=True, all_in_gpu=all_in_gpu,
            mixed_precision=mixed_precision)
        predict = torch.argmax(pred_test, 1)
        with open(join(os.path.dirname(output_filenames[0]), 'class.txt'), 'a') as f:
            f.write(f"idx:{idx},label:{label},pre:{predict}"+"\n")
        total += 1
        correct += torch.eq(predict, label).sum().double().item()
        score_test_pre.extend(pred_test.cpu().numpy())
    ...
    test_labels, score_test_pre[:, 1], average='weighted')
    test_ave_precision = metrics.average_precision_score(
        test_labels, score_test_pre[:, 1], average='weighted')
    ...
    result_string = "test_acc: {:.3f}\n" \
                "test_sens: {:.3f}\n" \
                "test_auc: {:.3f}\n" \
                "test_ave_precision: {:.3f}\n" \
                "test_F1: {:.3f}\n" \
                "test_recall: {:.3f}\n" \
                "test_precision: {:.3f}\n" \
                "test_spec : {:.3f}".format(test_acc, test_sens, test_auc, test_ave_precision, F1, recall, precision, test_spec)
    with open(join(os.path.dirname(output_filenames[0]), 'class.txt'), 'a') as f:
